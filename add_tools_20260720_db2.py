...
def main():
    ...
    new_tools = [
        ...
        },
        {
            ...
            "category": "ljud_och_video",
            "description": "Avancerad text-till-video AI som skapar högkvalitativa, jämna och realistiska 5-sekunders videoklipp utifrån prompter.",
            "pricing": "Freemium",
            "rating": 4.7,
            "gdpr_status": "Oklart",
            "tags": ["video", "text-till-video", "generativ-ai", "luma"],
            "icon_emoji": "🎬",
            "icon_bg_color": "bg-indigo-100"
        }
    ]
    ...
    for tool in new_tools:
        # Check if exists by exact name match
        c.execute("SELECT id FROM tools WHERE name = ?", (tool["name"],))
        result = c.fetchone()
        
        # also handle case-insensitive or partial
        if result is None:
            ...
            print(f"Tool {tool['name']} fanns redan.")
