...
def add_tools():
    ...
    # Handle the fact that tools.json might be a dict with "tools" key
    if isinstance(data, dict) and "tools" in data:
        data = data["tools"]

    # Filter out duplicates from json first
    seen = set()
    deduped_data = []
    for t in data:
    ...
    existing_names = {t.get("name") for t in deduped_data}
    
    added_count = 0
    for tool in TOOLS:
        if tool["name"] not in existing_names:
            deduped_data.append(tool)
            added_count += 1
